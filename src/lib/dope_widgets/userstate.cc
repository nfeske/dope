...
static User_state curr_state = USERSTATE_IDLE; /* current user state         */
...
static char       keytab[Input::KEY_MAX];
...
enum {
	USERSTATE_KEY_IDLE   = 0x0,  /* no key pressed          */
	USERSTATE_KEY_PRESS  = 0x1,  /* key pressed             */
	USERSTATE_KEY_REPEAT = 0x2,  /* long press - key repeat */
};
...
{
	return (keycode >= Input::BTN_LEFT && keycode <= Input::BTN_MIDDLE);
}
...
{
	long switches=0;
	if (keycode >= Input::KEY_MAX) return 0;
...
static void handle(void)
{
	...
	while (input->get_event(&event)) {
		...
		case EVENT_PRESS:
			...
			if (event.code == Input::BTN_LEFT)  curr_mb = curr_mb | 0x01;
			if (event.code == Input::BTN_RIGHT) curr_mb = curr_mb | 0x02;
			keytab[event.code] = 1;
			if (get_ascii(event.code)
			 || (event.code >= Input::KEY_UP && event.code <= Input::KEY_DELETE)) {
		...
		case EVENT_RELEASE:
			...
			if (event.code == Input::BTN_LEFT)  curr_mb = curr_mb & 0x00fe;
			if (event.code == Input::BTN_RIGHT) curr_mb = curr_mb & 0x00fd;
			keytab[event.code] = 0;
			curr_keystate = USERSTATE_KEY_IDLE;
			curr_keycode  = 0;
			break;
...
int init_userstate(struct dope_services *d)
{
	input   = (input_services  *)(d->get_module("Input 1.0"));
	...
	keymap  = (keymap_services *)(d->get_module("Keymap 1.0"));
