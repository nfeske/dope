 ...
 * Get widget type
 */
static char const * wid_get_type(WIDGET *w)
{
	return "unknown";
}
...
static void wid_set_x(WIDGET *w,long value)
{
	w->wd->x = value;
}
...
static void wid_set_y(WIDGET *w,long value)
{
	w->wd->y = value;
}
...
static void wid_set_w(WIDGET *w,long value)
{
	if (value < w->wd->min_w) value = w->wd->min_w;
	if (value > w->wd->max_w) value = w->wd->max_w;
	if (w->wd->w != value)
...
static void wid_set_h(WIDGET *w,long value)
{
	if (value < w->wd->min_h) value = w->wd->min_h;
	if (value > w->wd->max_h) value = w->wd->max_h;
	if (w->wd->h != value)
		w->wd->update |= WID_UPDATE_SIZE;
	w->wd->h = value;
}
...
static void wid_set_state(WIDGET *w,int value)
{
	if (value) w->wd->flags = w->wd->flags | WID_FLAGS_STATE;
...
static void wid_set_mfocus(WIDGET *w, int value)
{
	if (value) w->wd->flags = w->wd->flags | WID_FLAGS_MFOCUS;
...
static void wid_set_kfocus(WIDGET *w, int value)
{
	if (value) w->wd->flags = w->wd->flags | WID_FLAGS_KFOCUS;
...
static void wid_set_evforward(WIDGET *w,int value)
{
	if (value) w->wd->flags = w->wd->flags | WID_FLAGS_EVFORWARD;
 ...
 */
static void wid_bind(WIDGET *cw, char const *bind_ident, char const *message)
{
	struct binding *binding;

	INFO(printf("Widman(bind): create new binding for %s\n",bind_ident);)
	binding = (struct binding *)zalloc(sizeof(struct binding));
	if (!binding) {
		ERROR(printf("WidgetManager(bind): out of memory!\n");)
		return;
	}

	binding->msg        = strdup(message);
	binding->next       = cw->wd->bindings;
	binding->bind_ident = strdup(bind_ident);
	cw->wd->bindings    = binding;

	if (streq(bind_ident, "press",     6)) {binding->ev_type = EVENT_PRESS;       }
	if (streq(bind_ident, "release",   8)) {binding->ev_type = EVENT_RELEASE;     }
	if (streq(bind_ident, "motion",    7)) {binding->ev_type = EVENT_MOTION;      }
	if (streq(bind_ident, "leave",     6)) {binding->ev_type = EVENT_MOUSE_LEAVE; }
	if (streq(bind_ident, "enter",     6)) {binding->ev_type = EVENT_MOUSE_ENTER; }
	if (streq(bind_ident, "keyrepeat", 9)) {binding->ev_type = EVENT_KEY_REPEAT;  }

	if (binding->ev_type == 0)
		binding->ev_type = EVENT_ACTION;

	/* enable widget to receive the keyboard focus */
	if (binding->ev_type == EVENT_PRESS
	 || binding->ev_type == EVENT_RELEASE
	 || binding->ev_type == EVENT_KEY_REPEAT) {
 ...
 */
static void wid_unbind(WIDGET *cw, char const *bind_ident)
{
	struct binding *b = cw->wd->bindings;

	/* search for binding to remove */
	for (; b && b->next && streq(b->next->bind_ident, bind_ident, 16); b = b->next);
 ...
 */
static char const *wid_get_bind_msg(WIDGET *cw, char const *bind_ident)
{
	struct binding *cb;

	for (cb = cw->wd->bindings; cb; cb = cb->next)
		if (streq(cb->bind_ident,bind_ident, 256))
			return cb->msg;
...
static void build_script_lang(widtype *widtype, struct widget_methods *m)
{
	script->reg_widget_attrib(widtype, "string type",       (void *)m->get_type,      NULL,             NULL);
	script->reg_widget_attrib(widtype, "boolean state",     (void *)m->get_state,     (void *)m->set_state,     (void *)m->update);
	script->reg_widget_attrib(widtype, "boolean grabfocus", (void *)m->get_grabfocus, (void *)m->set_grabfocus, (void *)m->update);
	script->reg_widget_method(widtype, "void bind(string type,string msg)", (void *)m->bind);
	script->reg_widget_method(widtype, "void unbind(string type)", (void *)m->unbind);
	script->reg_widget_method(widtype, "void focus()", (void *)m->focus);
}
