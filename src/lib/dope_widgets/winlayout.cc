...
static WIDGET *new_button(WIDGET *next, long x, long y, long w, long h,
                          const char *txt, void *clic, long context)
{
...
static WIDGET *create_win_elements(s32 elements, int x, int y, int width, int height)
{
	...
	if (elements & WIN_BORDERS) {
		first=new_button(first,dx,dy+bsize,bsize,dh-bsize-bsize,NULL,(void *)resize_callback,WE_L);
		first=new_button(first,dx,dy,bsize,bsize,NULL,(void *)resize_callback,WE_L+WE_O);
		first=new_button(first,dx+bsize,dy,dw-bsize-bsize,bsize,NULL,(void *)resize_callback,WE_O);
		first=new_button(first,dx+dw-bsize,dy,bsize,bsize,NULL,(void *)resize_callback,WE_O+WE_R);
		first=new_button(first,dx+dw-bsize,dy+bsize,bsize,dh-bsize-bsize,NULL,(void *)resize_callback,WE_R);
		first=new_button(first,dx+dw-bsize,dy+dh-bsize,bsize,bsize,NULL,(void *)resize_callback,WE_R+WE_U);
		first=new_button(first,dx+bsize,dy+dh-bsize,dw-2*bsize,bsize,NULL,(void *)resize_callback,WE_U);
		first=new_button(first,dx,dy+dh-bsize,bsize,bsize,NULL,(void *)resize_callback,WE_L+WE_U);
		dx+=bsize;dy+=bsize;dw-=bsize*2;dh-=bsize*2;
	}

	if (elements & WIN_CLOSER) {
		first=new_button(first,dx,dy,tsize,tsize,"",(void *)close_callback,WE_CLOSER);
		dx+=tsize;dw-=tsize;
	}
	...
	if ((elements & WIN_TITLE) | (elements & WIN_CLOSER) | (elements & WIN_FULLER)) {
		first=new_button(first,dx,dy,dw,tsize,"DOpE WINDOW",(void *)move_callback,WE_TITLE);
		dy+=tsize;dh-=tsize;
	}
...
static void set_win_title(WIDGET *elem, char const *new_title)
{
	while (elem) {
...
static char const *get_win_title(WIDGET *elem)
{
	while (elem) {
		if ((unsigned long)elem->gen->get_context(elem) == WE_TITLE) {
