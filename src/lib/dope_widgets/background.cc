 ...
 */
static char const *bg_get_type(BACKGROUND *bg)
{
	return "Background";
}
...
{
	BACKGROUND *bg = ALLOC_WIDGET(struct background);
	SET_WIDGET_DEFAULTS(bg, struct background, &bg_methods);

	/* set background specific default attributes */
	bg->bd->style  =  BG_STYLE_WIN;
	bg->wd->flags |=  WID_FLAGS_CONCEALING;
...
static void build_script_lang(void)
{
	...
	script->reg_widget_attrib(widtype, "Widget content", NULL,
	                          (void *)bg_set_content, (void *)gen_methods.update);
