...
struct loadbar;
struct loadbar {
	char     const *ident;   /* identifier for bar                              */
...
enum {
	LOADDISPLAY_SIZE = 16,
	LOADDISPLAY_UPDATE_ORIENT = 0x01,
	LOADDISPLAY_UPDATE_VALUES = 0x02,
};

enum { NUM_DEFAULT_COLORS = 4 };
static color_t default_colors[NUM_DEFAULT_COLORS] = {
	GFX_RGBA(255U, 0U,   0U, 127U),
	GFX_RGBA(  0U, 0U, 255U, 127U),
	GFX_RGBA(  0U, 200U, 0U, 127U),
	GFX_RGBA(200U, 200U, 0U, 127U)
};
...
static struct loadbar *get_loadbar(struct loaddisplay *ld, char const *ident)
{
	...
	while (curr) {
		if (streq(ident, curr->ident, 256)) break;
		curr = curr->next;
	}
...
static color_t get_color_by_name(const char *colname)
{
	...
	for (i = 0; i < COLMAP_SIZE; i++) {
		if (streq(colmap[i].name, colname, 255)) return colmap[i].value;
	}
 ...
 */
static char const *ld_get_type(LOADDISPLAY *ld)
{
	return "LoadDisplay";
}
...
static void ld_set_orient(LOADDISPLAY *ld, char const *orient)
{
...
static char const *ld_get_orient(LOADDISPLAY *ld)
{
...
static void ld_barconfig(LOADDISPLAY *ld, char const *ident, char const *value,
                         char const *color)
{
	struct loadbar *lb;
	...
	if (!streq("<none>", value, 7)) {
		lb->value = atof(value);
	}

	if (!streq("<default>", color, 9)) {
...
{
	LOADDISPLAY *ld = ALLOC_WIDGET(struct loaddisplay);
	SET_WIDGET_DEFAULTS(ld, struct loaddisplay, &ld_methods);

	ld->wd->min_w = ld->wd->min_h = ld->wd->max_h = LOADDISPLAY_SIZE;
...
static void build_script_lang(void)
{
	...
	script->reg_widget_method(widtype, "void barconfig(string ident,string value=\"<none>\",string color=\"<default>\")", (void *)ld_barconfig);
	script->reg_widget_attrib(widtype, "string orient", (void *)ld_get_orient, (void *)ld_set_orient, (void *)gen_methods.update);
	script->reg_widget_attrib(widtype, "float from", (void *)ld_get_from, (void *)ld_set_from, (void *)gen_methods.update);
	script->reg_widget_attrib(widtype, "float to", (void *)ld_get_to, (void *)ld_set_to, (void *)gen_methods.update);
