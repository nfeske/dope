...
{
	struct section *section = (struct section *)zalloc(sizeof(struct section));
...
{
	struct section *section = new_section(idx);
	struct section *curr;

	if (!section) return NULL;

	/* if section list has no elements yet, set first element */
	if (*head == NULL) return *head = section;
	...
	/* insert new element */
	section->next = curr->next;
	return curr->next = section;
}
 ...
 */
static void erase_overlapping_cells(GRID *g, struct cell *cell)
{
	...
	while (cc) {
		...
		if ((cc->row) && (cc->row->index == cell->row->index)
		 && (cc->col) && (cc->col->index == cell->col->index)
		 && (cc->wid) && (cc->wid != cell->wid)) {
 ...
 */
static char const *grid_get_type(GRID *g)
{
	return "Grid";
}
...
{
	GRID *grid = ALLOC_WIDGET(struct grid);
	SET_WIDGET_DEFAULTS(grid, struct grid, &grid_methods);

	/* set grid specific widget attributes */
	grid->wd->flags |=  WID_FLAGS_CONCEALING;
...
static void build_script_lang(void)
{
	...
	script->reg_widget_method(widtype, "void columnconfig(long index,float weight=-1.0,long size=-1)", (void *)script_column_config);
	script->reg_widget_method(widtype, "void rowconfig(long index,float weight=-1.0,long size=-1)", (void *)script_row_config);
	script->reg_widget_method(widtype, "void place(Widget child,long column=9999,long row=9999,long columnspan=9999,long rowspan=9999,long padx=9999,long pady=9999,string align=\"\")", (void *)script_place_widget);
	script->reg_widget_method(widtype, "void add(Widget child)", (void *)grid_add);
	script->reg_widget_method(widtype, "void remove(Widget child)", (void *)grid_remove);
