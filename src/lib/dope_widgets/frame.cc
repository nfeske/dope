...
enum {
	FRAME_MODE_SCRX = 0x04,   /* horizontal scrollbars               */
	FRAME_MODE_SCRY = 0x08,   /* vertical scrollbars                 */
	FRAME_MODE_HIDX = 0x10,   /* auto-hiding of horizontal scrollbar */
	FRAME_MODE_HIDY = 0x20,   /* auto-hiding of vertical scrollbar   */
	FRAME_MODE_BG   = 0x40,   /* draw background                     */
};
...
static void frame_updatepos(FRAME *f)
{
	...
	/* send resize event */
	{
		char const *m = f->gen->get_bind_msg(f,"resize");
 ...
 * Request widget type
 */
static char const *frame_get_type(FRAME *f)
{
	return "Frame";
}
...
{
	FRAME *frame = ALLOC_WIDGET(struct frame);
	SET_WIDGET_DEFAULTS(frame, struct frame, NULL);
	frame->wd->flags |= WID_FLAGS_CONCEALING;
	return frame;
}
...
static void build_script_lang(void)
{
	...
	script->reg_widget_attrib(widtype, "Widget content", (void *)frame_get_content, (void *)frame_set_content, (void *)gen_methods.update);
	script->reg_widget_attrib(widtype, "boolean background", (void *)frame_get_background, (void *)frame_set_background, (void *)gen_methods.update);
	script->reg_widget_attrib(widtype, "boolean scrollx", (void *)frame_get_scrollx, (void *)frame_set_scrollx, (void *)gen_methods.update);
	script->reg_widget_attrib(widtype, "boolean scrolly", (void *)frame_get_scrolly, (void *)frame_set_scrolly, (void *)gen_methods.update);
	script->reg_widget_attrib(widtype, "long xview", (void *)frame_get_xview, (void *)frame_set_xview, (void *)gen_methods.update);
	script->reg_widget_attrib(widtype, "long yview", (void *)frame_get_yview, (void *)frame_set_yview, (void *)gen_methods.update);
	script->reg_widget_attrib(widtype, "long workw", (void *)frame_get_work_w, NULL, NULL);
	script->reg_widget_attrib(widtype, "long workh", (void *)frame_get_work_h, NULL, NULL);
	script->reg_widget_method(widtype, "void place(Widget child,long x=-999999,long y=-999999,long w=-999999,long h=-999999)", (void *)frame_place);
	script->reg_widget_method(widtype, "void expose(long x, long y)", (void *)frame_expose);
...
int init_frame(struct dope_services *d)
{
	...
	scroll  = (scrollbar_services  *)(d->get_module("Scrollbar 1.0"));
	bg      = (background_services *)(d->get_module("Background 1.0"));
	...
	tick    = (tick_services       *)(d->get_module("Tick 1.0"));
	relax   = (relax_services      *)(d->get_module("Relax 1.0"));
