...
enum {
	WIN_UPDATE_NEW_CONTENT = 0x01,
	WIN_UPDATE_SET_STAYTOP = 0x02,
	WIN_UPDATE_ELEMENTS    = 0x04,
	WIN_UPDATE_USERPOS     = 0x08,

	WIN_FLAGS_STAYTOP      = 0x01,
	WIN_FLAGS_BACKGROUND   = 0x02,
};
...
static void win_move_motion_callback(WIDGET *cw, int dx, int dy)
{
	...
	if (dx || dy) {
		char const *m = curr_window->gen->get_bind_msg((WIDGET *)curr_window, "move");
...
static void win_move_leave_callback(WIDGET *cw, int dx, int dy)
{
	...
	if (dx || dy) {
		char const *m  = curr_window->gen->get_bind_msg((WIDGET *)curr_window, "moved");
		s32         id = curr_window->gen->get_app_id((WIDGET *)curr_window);
...
static void win_resize_leave_callback(WIDGET *cw, int dx, int dy)
{
	...
	if (dx || dy) {
		char const *m = curr_window->gen->get_bind_msg((WIDGET *)curr_window, "resized");
		s32        id = curr_window->gen->get_app_id((WIDGET *)curr_window);
...
static void win_set_app_id(WINDOW *w, s32 app_id)
{
	...
	char const *app_name = appman->get_app_name(app_id);
...
static void win_handle_event(WIDGET *w, EVENT *ev, WIDGET *from)
{
	...
	if ((ev->type != EVENT_PRESS) || (ev->code != Input::BTN_LEFT)) return;
...
static char const *win_get_type(WINDOW *w)
{
	return "Window";
}
...
static void win_set_title(WINDOW *w, char const *new_title)
{
	SCREEN *scr = (SCREEN *)w->gen->get_parent(w);
...
static char const *win_get_title(WINDOW *w)
{
	return winlayout->get_win_title(w->wind->elem);
}


static void win_set_bgcolor(WINDOW *w, char const *new_bgcolor)
{
	if (!new_bgcolor) return;
	if (streq(new_bgcolor, "black", 255))
...
{
	WINDOW *win = ALLOC_WIDGET(struct window);
	SET_WIDGET_DEFAULTS(win, struct window, &win_methods);
	...
	win->wind->flags    =  WIN_FLAGS_BACKGROUND;
	win->wind->elements =  WIN_CLOSER  | WIN_TITLE | WIN_BORDERS;
	win->wind->ux = win->wind->uy = win->wind->uw = win->wind->uh = NOARG;
	win->wind->bgcol = 0x708090ff;

	/* create window elements */
	win->wind->elem = winlayout->create_win_elements(win->wind->elements,
	                                                 shadow_left, shadow_top,
	                                                 win->wd->w - shadow_left - shadow_right,
	                                                 win->wd->h - shadow_top - shadow_bottom);
	/* tell all window elements about its proud parent */
	adopt_win_elements(win, win->wind->elem);
...
{
	widtype *widtype = script->reg_widget_type("Window", (void *(*)(void))create);

	script->reg_widget_method(widtype, "void open(void)",  (void *)win_open);
	script->reg_widget_method(widtype, "void close(void)", (void *)win_close);
	script->reg_widget_method(widtype, "void top(void)",   (void *)win_top);
	script->reg_widget_method(widtype, "void back(void)",  (void *)win_back);

	script->reg_widget_attrib(widtype, "Widget content", (void *)win_get_content, (void *)win_set_content, (void *)win_update);
	script->reg_widget_attrib(widtype, "boolean staytop", (void *)win_get_staytop, (void *)win_set_staytop, (void *)win_update);
	script->reg_widget_attrib(widtype, "boolean background", (void *)win_get_background, (void *)win_set_background, (void *)win_update);
	script->reg_widget_attrib(widtype, "string title", (void *)win_get_title, (void *)win_set_title, NULL);
	script->reg_widget_attrib(widtype, "string bgcolor", NULL, (void *)win_set_bgcolor, NULL);
	script->reg_widget_attrib(widtype, "long x", (void *)win_get_x, (void *)win_set_x, (void *)win_update);
	script->reg_widget_attrib(widtype, "long y", (void *)win_get_y, (void *)win_set_y, (void *)win_update);
	script->reg_widget_attrib(widtype, "long w", (void *)win_get_w, (void *)win_set_w, (void *)win_update);
	script->reg_widget_attrib(widtype, "long h", (void *)win_get_h, (void *)win_set_h, (void *)win_update);
	script->reg_widget_attrib(widtype, "long workx", (void *)win_get_workx, (void *)win_set_workx, (void *)win_update);
	script->reg_widget_attrib(widtype, "long worky", (void *)win_get_worky, (void *)win_set_worky, (void *)win_update);
	script->reg_widget_attrib(widtype, "long workw", (void *)win_get_workw, (void *)win_set_workw, (void *)win_update);
	script->reg_widget_attrib(widtype, "long workh", (void *)win_get_workh, (void *)win_set_workh, (void *)win_update);
...
int init_window(struct dope_services *d)
{
	...
	winlayout = (winlayout_services *)(d->get_module("WinLayout 1.0"));
	...
	gen_shadow((u32 *)gfx->map(shadow), shadow_w, shadow_h, shadow_w >> 1, shadow_h >> 1);
