...
static void create_desktop(SCREEN *scr)
{
	...
	viewman->set_bg((view *)(desk->wd->context));
 ...
 */
static char const *scr_get_type(SCREEN *s)
{
	return "Screen";
}
...
{
	WINDOW *new_win = (WINDOW *)w, *old;
	BUTTON *b;
	char const *new_txt = "";

	if (new_win == (WINDOW *)scr->sd->active_win) return;

	if (new_win) {
		if (new_win->win->get_staytop(new_win)) return;
		new_win->gen->inc_ref((WIDGET *)new_win);
		new_win->win->set_state(new_win, 1);
	}
	...
	scr->sd->active_win = (WIDGET *)new_win;

	/* use title of new focused application for menu bar */
	if (new_win) {
		int app_id = new_win->gen->get_app_id((WIDGET *)new_win);
...
static void scr_place(SCREEN *scr, WIDGET *ww, long x, long y, long w, long h)
{
	...
	if (ww->wd->context)
		viewman->set_title((view *)ww->wd->context,
		                   appman->get_app_name(ww->gen->get_app_id(ww)));

	/* update view */
	viewman->place((view *)ww->wd->context, x + win->shadow_left, y + win->shadow_top,
...
static void scr_remove(SCREEN *scr, WIDGET *win)
{
	...
	viewman->destroy((view *)win->wd->context);
	win->wd->context = 0;
...
static void scr_top(SCREEN *scr, WIDGET *win)
{
	...
	/* notify view manager */
	viewman->top((view *)win->wd->context);
...
static void scr_back(SCREEN *scr, WIDGET *win)
{
	...
	/* notify view manager */
	viewman->back((view *)win->wd->context);
	viewman->set_bg((view *)win->wd->context);
 ...
 */
static void scr_set_title(SCREEN *scr, WIDGET *win, char const *title)
{
	if (!win || (win->wd->parent != scr)) return;

	/* notify view manager */
	viewman->set_title((view *)win->wd->context, title);
}
...
{
	SCREEN *screen = ALLOC_WIDGET(struct screen);
	SET_WIDGET_DEFAULTS(screen, struct screen, &scr_methods);

	/* insert screen into screen list */
	if (!first_scr)
		first_scr = screen;
	else {
		screen->sd->next = first_scr->sd->next;
		first_scr = screen;
	}
	return screen;
}
...
static void build_script_lang(void)
{
	...
	script->reg_widget_attrib(widtype, "long w", (void *)scr_get_w, NULL, NULL);
	script->reg_widget_attrib(widtype, "long h", (void *)scr_get_h, NULL, NULL);
	script->reg_widget_method(widtype, "void refresh()", (void *)scr_refresh);
...
int init_screen(struct dope_services *d)
{
	...
	viewman   = (viewman_services    *)(d->get_module("ViewManager 1.0"));
	...
	frame     = (frame_services      *)(d->get_module("Frame 1.0"));
	gfx       = (gfx_services        *)(d->get_module("Gfx 1.0"));
	cont      = (container_services  *)(d->get_module("Container 1.0"));
	but       = (button_services     *)(d->get_module("Button 1.0"));
	win       = (window_services     *)(d->get_module("Window 1.0"));
