...
enum { VTEXTSCR_MODE_C8A8PLN = 1 }; /* 8bit characters, 8bit attributes, planar */
 ...
 */
static char const *vtextscr_get_type(VTEXTSCREEN *vs)
{
	return "VTextScreen";
}
...
{
	if (!streq(mode, "C8A8PLN", 7)) return 0;
...
static s32 vtextscr_set_mode(VTEXTSCREEN *vts, s32 width, s32 height, char *mode)
{
	...
	if (streq("C8A8PLN", mode, 7)) {
		...
		vts->vd->tmpstr = (char *)malloc(width + 3);
	}
	...
	vts->vd->xres = width;
	vts->vd->yres = height;
	vts->vd->buffer = (u8 *)(shmem->get_address(vts->vd->smb));
 ...
 */
static char const *vtextscr_map(VTEXTSCREEN *vts, char *dst_thread_ident)
{
	char dst_th_buf[16];
...
{
	VTEXTSCREEN *vtxt = ALLOC_WIDGET(struct vtextscreen);
	SET_WIDGET_DEFAULTS(vtxt, struct vtextscreen, &vtextscreen_methods);

	/* set widget type specific data */
	set_default_values(vtxt);
	vtxt->wd->flags |= WID_FLAGS_CONCEALING | WID_FLAGS_EDITABLE | WID_FLAGS_HIGHLIGHT;
...
static void build_script_lang(void)
{
	...
	script->reg_widget_method(widtype, "long probemode(long width,long height,string mode)", (void *)vtextscr_probe_mode);
	script->reg_widget_method(widtype, "long setmode(long width,long height,string mode)", (void *)vtextscr_set_mode);
	script->reg_widget_method(widtype, "string map(string thread=\"caller\")", (void *)vtextscr_map);
	script->reg_widget_method(widtype, "void refresh(long x=0, long y=0, long w=-1, long h=-1)", (void *)vtextscr_refresh);
	script->reg_widget_attrib(widtype, "long charw", (void *)vtextscr_get_char_w, NULL, NULL);
	script->reg_widget_attrib(widtype, "long charh", (void *)vtextscr_get_char_h, NULL, NULL);
	script->reg_widget_attrib(widtype, "long cursorx", (void *)vtextscr_get_cursor_x, (void *)vtextscr_set_cursor_x, (void *)gen_methods.update);
	script->reg_widget_attrib(widtype, "long cursory", (void *)vtextscr_get_cursor_y, (void *)vtextscr_set_cursor_y, (void *)gen_methods.update);
