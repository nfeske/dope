 ...
 */
enum {
	WID_DRAW_TRANS = 0x1, /* draw transparent widget elements */
	WID_DRAW_SOLID = 0x2, /* draw solid widget elements       */
};
...
struct widget_methods {
	 ...
	 *
	 * It is called implicitly when the widget gets destroyed.
	 */
	void (*free_data) (WIDGETARG *);

	char const *(*get_type) (WIDGETARG *);
	...
	void        (*bind)         (WIDGETARG *, char const *bind_ident,
	                                          char const *message);
	void        (*unbind)       (WIDGETARG *, char const *bind_ident);
	char const *(*get_bind_msg) (WIDGETARG *, char const *bind_ident);
