...
enum {
	SCROLLBAR_NUM_ELEM = 4, /* number of scrollbar element widgets */
	SB_ELEM_INC_ARROW  = 0, /* right/bottom arrow                  */
	SB_ELEM_DEC_ARROW  = 1, /* left/top arrow                      */
	SB_ELEM_SLIDER_FG  = 2, /* slider                              */
	SB_ELEM_SLIDER_BG  = 3, /* background of the slider            */

	SCROLLBAR_VER      = 0x1,
	SCROLLBAR_AUTOVIEW = 0x2,
};
...
static void scrollbar_set_view_offset (SCROLLBAR *s, s32 new_view_offset)
{
	...
	{
		char const *message = s->gen->get_bind_msg(s, "change");
...
static void scrollbar_set_orient(SCROLLBAR *s, char const *orient)
{
...
static char const *scrollbar_get_orient(SCROLLBAR *s)
{
...
static WIDGET *new_button(WIDGET *parent, char *txt, void *clic, long context, int state)
{
	...
	nb->but->set_click(nb, (void (*)(button*))clic);
...
{
	SCROLLBAR *sb = ALLOC_WIDGET(struct scrollbar);
	SET_WIDGET_DEFAULTS(sb, struct scrollbar, &scroll_methods);

	/* default scrollbar attributes */
	sb->sd->type          = SCROLLBAR_AUTOVIEW;
	sb->sd->arrow_size    = 13;
	sb->sd->real_size     = 500;
	sb->sd->view_size     = 128;
	sb->sd->view_offset   = 0;
	sb->sd->step_size     = 16;
	sb->sd->scroll_update = NULL;

	/* create scrollbar element widgets */
	sb->sd->elem[SB_ELEM_INC_ARROW] = new_button(sb, NULL, (void *)arrow_callback,  0, 0);
	sb->sd->elem[SB_ELEM_DEC_ARROW] = new_button(sb, NULL, (void *)arrow_callback,  1, 0);
	sb->sd->elem[SB_ELEM_SLIDER_BG] = new_button(sb, NULL, (void *)slidbg_callback, 0, 1);
	sb->sd->elem[SB_ELEM_SLIDER_FG] = new_button(sb, NULL, (void *)slider_callback, 0, 0);

	sb->wd->flags |= WID_FLAGS_CONCEALING;
...
static void build_script_lang(void)
{
	...
	script->reg_widget_attrib(widtype, "string orient",    (void *)scrollbar_get_orient,      (void *)scrollbar_set_orient,      (void *)gen_methods.update);
	script->reg_widget_attrib(widtype, "long viewsize",    (void *)scrollbar_get_view_size,   (void *)scrollbar_set_view_size,   (void *)gen_methods.update);
	script->reg_widget_attrib(widtype, "long realsize",    (void *)scrollbar_get_real_size,   (void *)scrollbar_set_real_size,   (void *)gen_methods.update);
	script->reg_widget_attrib(widtype, "long offset",      (void *)scrollbar_get_view_offset, (void *)scrollbar_set_view_offset, (void *)gen_methods.update);
	script->reg_widget_attrib(widtype, "boolean autoview", (void *)scrollbar_get_autoview,    (void *)scrollbar_set_autoview,    (void *)gen_methods.update);
...
int init_scrollbar(struct dope_services *d)
{
	but       = (button_services    *)(d->get_module("Button 1.0"));
