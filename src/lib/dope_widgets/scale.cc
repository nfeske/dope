...
static WIDGET *new_button(char const *txt, void *clic, long context)
{
	BUTTON *nb = (BUTTON *)(but->create());
 ...
 */
static void (*orig_bind)(WIDGET *w, char const *bind_ident, char const *message);
static void scale_bind(SCALE *s, char const *bind_ident, char const *message)
{
	WIDGET *cw;
 ...
 */
static char const *scale_get_type(SCALE *s)
{
	return "Scale";
}
...
{
	char const *m = curr_scale->gen->get_bind_msg(curr_scale, "slid");
...
static void slider_bg_callback(BUTTON *b)
{
	...
	printf("slider_bg_callback called (dir=%d), not implemented\n", dir);
...
{
	SCALE *scale = ALLOC_WIDGET(struct scale);
	SET_WIDGET_DEFAULTS(scale, struct scale, NULL);
	...
	scale->sd->slider    = new_button(NULL, (void *)slider_callback, 0);
	scale->sd->slider_bg = new_button(NULL, (void *)slider_bg_callback, 0);

	scale->sd->slider_bg->gen->set_state(scale->sd->slider_bg, 1);
	scale->sd->slider->gen->set_parent(scale->sd->slider, scale);
	scale->sd->slider_bg->gen->set_parent(scale->sd->slider_bg, scale);

	refresh_elements(scale);
	scale_calc_minmax(scale);
	return scale;
}
...
static char const *scale_get_orient(SCALE *s)
{
...
static void build_script_lang(void)
{
	...
	script->reg_widget_attrib(widtype,"Variable variable", (void *)scale_get_var, (void *)scale_set_var, (void *)gen_methods.update);
	script->reg_widget_attrib(widtype,"float value", (void *)scale_get_value, (void *)scale_set_value, (void *)gen_methods.update);
	script->reg_widget_attrib(widtype,"float from", (void *)scale_get_from, (void *)scale_set_from, (void *)gen_methods.update);
	script->reg_widget_attrib(widtype,"float to", (void *)scale_get_to, (void *)scale_set_to, (void *)gen_methods.update);
	script->reg_widget_attrib(widtype,"string orient", (void *)scale_get_orient, (void *)scale_set_orient, (void *)gen_methods.update);
