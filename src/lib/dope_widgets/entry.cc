...
static int delete_char(ENTRY *e, int idx)
{
	...
	if (idx >= (int)strlen(e->ed->txtbuf)) return 0;
	while (*src) *(dst++) = *(src++);
	*dst = 0;
	return 1;
}
...
static int insert_char(ENTRY *e, int idx, char c)
{
	...
	if (len >= e->ed->txtbuflen) {
		...
		buf = (char *)zalloc(e->ed->txtbuflen);
		if (!buf) return 0;
		for(i=0; i<=len; i++) buf[i] = e->ed->txtbuf[i];
		free(e->ed->txtbuf);
		e->ed->txtbuf = buf;
	}
	dst = e->ed->txtbuf + len;
...
{
	if (e->ed->flags & ENTRY_FLAGS_BLIND) {
		unsigned step = font->calc_str_width(e->ed->font_id, "*");
...
static void entry_handle_event(ENTRY *e, EVENT *ev, WIDGET *from)
{
	...
	case EVENT_KEY_REPEAT:
		...
		case Input::KEY_RIGHT:
			if (e->ed->curpos < (int)strlen(e->ed->txtbuf)) e->ed->curpos++;
			ev_done = 2;
			break;
		...
		case Input::KEY_DELETE:
			if (e->ed->curpos < (int)strlen(e->ed->txtbuf))
		...
		case Input::KEY_ENTER:
			...
			{
				char const *m = e->gen->get_bind_msg(e, "commit");
 ...
 */
static char const *entry_get_type(ENTRY *e)
{
	return "Entry";
}
...
{
	ENTRY *entry = ALLOC_WIDGET(struct entry);
	SET_WIDGET_DEFAULTS(entry, struct entry, &entry_methods);

	/* set entry specific attributes */
	entry->ed->pad_x     = entry->ed->pad_y = 2;
	entry->ed->vislen    = 2;
	entry->ed->sel_beg   = -1;
	entry->ed->sel_end   = -1;
	entry->ed->txtbuflen = 16;
	entry->ed->txtbuf    = (char *)zalloc(entry->ed->txtbuflen);
	entry->wd->flags    |= WID_FLAGS_EDITABLE | WID_FLAGS_HIGHLIGHT;

	/* let the entry receive the keyboard focus even without any bindings */
	entry->wd->flags    |= WID_FLAGS_TAKEFOCUS;

	update_text_pos(entry);
	entry_calc_minmax(entry);
	return entry;
}
...
static void build_script_lang(void)
{
	...
	script->reg_widget_attrib(widtype, "string text", (void *)entry_get_text, (void *)entry_set_text, (void *)gen_methods.update);
	script->reg_widget_attrib(widtype, "boolean blind", (void *)entry_get_blind, (void *)entry_set_blind, (void *)gen_methods.update);
